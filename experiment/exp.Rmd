...
res.1 <- 'Data_viz_test_May-3-2018_08.45.csv'


vdf.1 <- read.csv(paste(path,res.1,sep=''))
vdf1.names <- c('ResponseId','Duration..in.seconds.','Q6','Q10','Q9')
vdf.1 <- vdf.1[(nrow(vdf.1) - 44):nrow(vdf.1),vdf1.names]
names(vdf.1) <- c('rID','duration','pizza.1st','homicide.2nd','rainfall.3rd')
vdf.1$duration <- as.numeric(vdf.1$duration)

vdf.1$pizza.1st <- as.character(vdf.1$pizza.1st)
vdf.1$homicide.2nd <- as.character(vdf.1$homicide.2nd)
vdf.1$rainfall.3rd <- as.character(vdf.1$rainfall.3rd)

ggplot(vdf.1) + geom_histogram(aes(duration),binwidth = 5)

# drop people guessing randomly
vdf1.sub <- vdf.1[vdf.1$duration >= 20, ]

# Make some bar charts
ggplot(vdf1.sub) + geom_bar(aes(pizza.1st)) + scale_y_continuous(limits = c(0,40))
ggplot(vdf1.sub) + geom_bar(aes(homicide.2nd)) + scale_y_continuous(limits = c(0,40))
ggplot(vdf1.sub) + geom_bar(aes(rainfall.3rd)) + scale_y_continuous(limits = c(0,40))
```
...
vdf1.sub <- right.wrong(vdf1.sub,'pizza.1st','North Dakota')
vdf1.sub <- right.wrong(vdf1.sub,'homicide.2nd','Utah')
vdf1.sub <- right.wrong(vdf1.sub,'rainfall.3rd','Virginia')
...
```{r vis_numbers}
ggplot(vdf1.sub) + geom_bar(aes(x=pizza.1st.right,y= (..count..)/sum(..count..),fill = pizza.1st, alpha=pizza.1st)) + 
...
ggplot(vdf1.sub) + geom_bar(aes(x=homicide.2nd.right,y= (..count..)/sum(..count..),fill = homicide.2nd, alpha=homicide.2nd)) + 
       scale_x_discrete(labels=c('Incorrect','Correct')) +
       scale_alpha_manual(values=c(.4,1,1)) + 
       theme(legend.position='bottom',legend.title=element_blank(),
            plot.title=element_text(hjust=0.5),
...
ggplot(vdf1.sub) + geom_bar(aes(x=factor(rainfall.3rd.right),y= (..count..)/sum(..count..),fill = factor(rainfall.3rd), alpha=factor(rainfall.3rd))) + 
       scale_x_discrete(labels=c('Incorrect','Correct')) +
       scale_alpha_manual(values=c(.4,1,1)) + 
       theme(legend.position='bottom',legend.title=element_blank(),
            plot.title=element_text(hjust=0.5),
