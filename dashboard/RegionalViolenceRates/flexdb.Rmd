...
zones$M5Dis[!zones$zone%in%c(8,14,20)] <- NA
zones$M6Dis[!zones$zone%in%c(2,3,5,7,18)] <- NA
zones$M7Dis[!zones$zone%in%c(1,6,7,10,11,12,13)] <- NA
```
...
m5.bins <- seq(min(na.omit(zones$M5Dis)),max(na.omit(zones$M5Dis)),by=(max(na.omit(zones$M5Dis)) - min(na.omit(zones$M5Dis)))/7)
m5.pal <- colorBin('YlOrRd', domain = zones$M5Dis, bins = m5.bins, reverse=FALSE)

m6.bins <- seq(min(na.omit(zones$M6Dis)),max(na.omit(zones$M6Dis)),by=(max(na.omit(zones$M6Dis)) - min(na.omit(zones$M6Dis)))/7)
m6.pal <- colorBin('YlOrRd', domain = zones$M6Dis, bins = m6.bins, reverse=FALSE)

m7.bins <- seq(min(na.omit(zones$M7Dis)),max(na.omit(zones$M7Dis)),by=(max(na.omit(zones$M7Dis)) - min(na.omit(zones$M7Dis)))/7)
m7.pal <- colorBin('YlOrRd', domain = zones$M7Dis, bins = m7.bins, reverse=FALSE)
```
...
<p align=center><font size=3>
Figure 7. Model C, OLS by climate regions: Disadvantage coefficient, violent crime rate of the sampled tracts in each region.</font><br>
```{r frame7}
zones.map %>% addPolygons(weight = 1, color = '#000000',opacity = 1.0,fill=TRUE,fillOpacity = 0.6,
                                                            fillColor = ~m5.pal(zones$M5Dis),
                                                            highlight = hopts,
                                                            label = m5.labels,
                                                            labelOptions = lopts) %>%
                                                addLegend(pal=m5.pal, values=~zones$M5Dis, opacity = 0.7, title = NULL,position='bottomright')
```
</p>
*** 
<p align=justify><font size=4>
...
Figure 7 shows that for the regions where non-spatial OLS is the correct specification, the role of disadvantage varies by climate region.<br><br>
</font></p>

### Model D: Spatial Lag by Regions (Disadvantage) {data-commentary-width=450}
<p align=center><font size=3>
Figure 8. Model D, spatial lag by regions: Disadvantage coefficient, violent crime rate of the sampled tracts in each region.</font><br>
```{r frame8}
zones.map %>% addPolygons(weight = 1, color = '#000000',opacity = 1.0,fill=TRUE,fillOpacity = 0.6,
                                                            fillColor = ~m6.pal(zones$M6Dis),
                                                            highlight = hopts,
                                                            label = m6.labels,
                                                            labelOptions = lopts) %>%
                                                addLegend(pal=m6.pal, values=~zones$M6Dis, opacity = 0.7, title = NULL,position='bottomright')
```
</p>
*** 
<p align=justify><font size=4>
...
```{r frame9}
zones.map %>% addPolygons(weight = 1, color = '#000000',opacity = 1.0,fill=TRUE,fillOpacity = 0.6,
                                                            fillColor = ~m7.pal(zones$M7Dis),
                                                            highlight = hopts,
                                                            label = m7.labels,
                                                            labelOptions = lopts) %>%
                                                addLegend(pal=m7.pal, values=~zones$M7Dis, opacity = 0.7, title = NULL,position='bottomright')
```
