...
zones$Coef5D[!zones$zone%in%c(8,14,20)] <- NA
zones$Coef6D[!zones$zone%in%c(2,3,5,7,18)] <- NA
zones$Coef7D[!zones$zone%in%c(1,6,7,10,11,12,13)] <- NA
```
...
m5.bins <- seq(min(na.omit(zones$Coef5D)),max(na.omit(zones$Coef5D)),by=(max(na.omit(zones$Coef5D)) - min(na.omit(zones$Coef5D)))/7)
m5.pal <- colorBin('YlOrRd', domain = zones$Coef5D, bins = m5.bins, reverse=FALSE)

m6.bins <- seq(min(na.omit(zones$Coef6D)),max(na.omit(zones$Coef6D)),by=(max(na.omit(zones$Coef6D)) - min(na.omit(zones$Coef6D)))/7)
m6.pal <- colorBin('YlOrRd', domain = zones$Coef6D, bins = m6.bins, reverse=FALSE)

m7.bins <- seq(min(na.omit(zones$Coef7D)),max(na.omit(zones$Coef7D)),by=(max(na.omit(zones$Coef7D)) - min(na.omit(zones$Coef7D)))/7)
...
<p align=center><font size=3>
Figure 7. Model C: Coefficient for disadvantage, regimes regression, for violent crime rate of the sampled tracts in each region.</font><br>
```{r frame7}
zones.map %>% addPolygons(weight = 1, color = '#000000',opacity = 1.0,fill=TRUE,fillOpacity = 0.6,
...
*** 
<p align=justify><font size=4>
<b>Disadvantage drives violent crime for tracts in some Southern subregions.</b><br><br>
